...
def ensemble(args):
    ...
    # Skip gracefully when the expected checkpoints aren't on disk (e.g. a
    # fast_dev_run, which disables checkpointing, or an interrupted run).
    last = [path for path in last if os.path.exists(path)]
    if not last:
        print("[ensemble] no epoch checkpoints found to average; skipping.")
        return None
