...
class DataModule(LightningDataModule):
    ...
    def _dataloader(self, ds, sampler, collate_fn):
        num_workers = self.cfg.data.get("num_workers", 12)
    ...
    def train_dataloader(self):
        ...
        # Random noise augmentation levels are configurable per experiment; the
        # defaults reproduce the original hardcoded behaviour (a mix of SNRs
        # from -5 dB up to a clean pass-through, applied to every utterance).
        snr_levels = self.cfg.data.get("noise_snr_levels", None)
        if snr_levels is not None:
            snr_levels = list(snr_levels)
        noise_prob = self.cfg.data.get("noise_prob", 1.0)
        train_ds = AVDataset(
            ...
            audio_transform=AudioTransform(
                "train", snr_levels=snr_levels, noise_prob=noise_prob
            ),
