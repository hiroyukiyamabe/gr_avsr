...
def load_video(path):
    ...
    with av.open(str(path)) as container:
        ...
        # PyAV's default multithreaded decoding segfaults (0xC0000005) inside
        # Windows "spawn" DataLoader workers; force single-threaded decode.
        stream.thread_type = "NONE"
        frames = [f.to_ndarray(format="rgb24") for f in container.decode(stream)]
    vid = torch.from_numpy(np.stack(frames))  # T x H x W x C, uint8
...
def load_audio(path):
    ...
    data, sample_rate = sf.read(path[:-4] + ".wav", dtype="float32", always_2d=True)
