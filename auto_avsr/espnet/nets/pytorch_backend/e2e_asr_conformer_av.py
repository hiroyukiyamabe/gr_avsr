...
from espnet.nets.pytorch_backend.nets_utils import MLPHead, ModalityGate
...
class E2E(torch.nn.Module):
    def __init__(self, odim, args, ignore_id=-1):
        ...
        # Noise-sensitive dynamic fusion (Option A: blind per-frame gating). The
        # gate predicts modality reliability from the features themselves, with no
        # ground-truth SNR needed at train or test time. Zero-initialised, so it
        # starts equivalent to the original static concatenation fusion.
        self.fusion_gate = ModalityGate(
            adim=args.adim,
            aux_adim=args.aux_adim,
            hdim=getattr(args, "fusion_gate_hdim", 256),
        )
        # Static-fusion ablation: freeze the gate at its zero-init. The neutral
        # (0.5, 0.5) output times the 2x scaling in fuse() is numerically
        # identical to the original concat fusion, so a frozen-gate run IS the
        # static baseline, trained through the exact same code path.
        if getattr(args, "fusion_gate_freeze", False):
    ...
    def fuse(self, video_feat, audio_feat):
        """Noise-sensitive dynamic fusion of frame-aligned features.

        Single source of truth for fusion, shared by training (``forward``) and
        inference (``lightning_av.py``). Returns the fused features and the
        per-frame gate weights ``[B, T, 2]`` (visual, audio) for inspection.

        Each modality is scaled by ``2 * weight`` so that the neutral gate state
        ``(0.5, 0.5)`` reproduces the original static concatenation exactly.
        """
        weights = self.fusion_gate(video_feat, audio_feat)  # [B, T, 2]
        w_v = weights[..., 0:1]
        w_a = weights[..., 1:2]
        weighted = torch.cat((2 * w_v * video_feat, 2 * w_a * audio_feat), dim=-1)
    ...
    def forward(self, video, audio, video_lengths, audio_lengths, label):
        ...
        x, gate_weights = self.fuse(video_feat, audio_feat)

        # Mean gate weight assigned to the audio stream over valid (non-pad)
        # frames -- logged so the audio reliance can be tracked against SNR.
        valid = video_padding_mask.squeeze(-2)  # [B, T] bool, True = real frame
        audio_weight = (gate_weights[..., 1] * valid).sum() / valid.sum()
        ...
        return loss, loss_ctc, loss_att, acc, audio_weight
