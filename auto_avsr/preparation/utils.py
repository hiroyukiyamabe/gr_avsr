...
def save2aud(filename, aud, sample_rate):
    os.makedirs(os.path.dirname(filename), exist_ok=True)
    # torchaudio >= 2.9 routes save() through torchcodec; write via soundfile.
    # aud is channels x frames; soundfile expects frames x channels.
    sf.write(filename, aud.t().cpu().numpy(), sample_rate, subtype="PCM_16")
